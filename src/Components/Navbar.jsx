...
const NAV_LINKS = ["Home", "Missions", "Planets", "Gallery", "Contact"];
  ...
  return (
    <>
      <style>{`
        ...
        .nav-link.is-hovered::before, .nav-link.is-active::before {
        ...
        @media (max-width: 768px) { .explore-btn { display: none; } }
        ...
        /* Warp particles on explore hover */
        .particle {
          position: absolute;
          width: 2px;
          height: 2px;
          ...
          background: #a78bfa;
          pointer-events: none;
          animation: warp-out 0.6s ease-out forwards;
        }
        @keyframes warp-out {
          from { transform: scale(1); opacity: 1; }
          to   { transform: scale(0) translateX(20px); opacity: 0; }
        }
        ...
        .nav-link-wrap:nth-child(1) .nav-link { animation: link-in 0.5s 0.2s both; }
              ...
              {NAV_LINKS.map(label => (
                <li key={label} className="nav-link-wrap">
                  <a
                    ...
                    className={`nav-link ${hovered === label ? "is-hovered" : ""} ${active === label ? "is-active" : ""}`}
