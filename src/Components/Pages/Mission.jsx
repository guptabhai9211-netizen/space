...
function StarField() {
  ...
  useEffect(() => {
    ...
    const stars = Array.from({ length: 250 }, () => ({
      ...
      r: Math.random() * 1.6 + 0.2, a: Math.random(), speed: Math.random() * 0.004 + 0.001,
    }));
    // shooting stars
    const shoots = Array.from({ length: 5 }, () => ({ x: -100, y: -100, vx: 0, vy: 0, life: 0, maxLife: 0 }));
    let animId;
    const spawnShoot = (s) => {
      ...
      s.life = 0; s.maxLife = Math.random() * 60 + 40;
    };
    ...
    function draw() {
      ...
      shoots.forEach((s) => {
        ...
        ctx.lineTo(s.x + s.vx * s.life - s.vx * 12, s.y + s.vy * s.life - s.vy * 12);
        const grad = ctx.createLinearGradient(
          s.x + s.vx * s.life, s.y + s.vy * s.life,
          s.x + s.vx * s.life - s.vx * 12, s.y + s.vy * s.life - s.vy * 12
        );
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none", overflow: "hidden" }}>
      <div style={{
        ...
        background: "radial-gradient(ellipse, rgba(74,158,255,0.07) 0%, transparent 65%)",
        animation: "nebulaDrift1 20s ease-in-out infinite",
      }} />
      <div style={{
        position: "absolute", bottom: "-10%", right: "-10%", width: "50%", height: "50%",
        background: "radial-gradient(ellipse, rgba(255,69,0,0.06) 0%, transparent 65%)",
        animation: "nebulaDrift2 25s ease-in-out infinite",
      }} />
      <div style={{
        position: "absolute", top: "30%", right: "20%", width: "40%", height: "40%",
        background: "radial-gradient(ellipse, rgba(123,200,255,0.04) 0%, transparent 65%)",
  ...
  return (
    <div
      ...
      style={{
        ...
        cursor: "pointer",
        minHeight: "320px",
        transform: isVisible ? "translateY(0) scale(1)" : "translateY(80px) scale(0.88)",
        opacity: isVisible ? 1 : 0,
        transition: `transform 0.7s cubic-bezier(0.23,1,0.32,1) ${index * 100}ms, opacity 0.7s ease ${index * 100}ms`,
      }}
    ...
    >
      <div style={{
        ...
        <div style={{
          ...
          background: hovered
            ? `linear-gradient(135deg, rgba(74,158,255,0.12) 0%, rgba(10,20,60,0.96) 100%)`
            : `linear-gradient(135deg, rgba(255,255,255,0.04) 0%, rgba(8,12,35,0.92) 100%)`,
          border: `1px solid ${hovered ? mission.color + "70" : "rgba(255,255,255,0.08)"}`,
          borderRadius: "20px",
          padding: "22px",
          display: "flex", flexDirection: "column", gap: "12px",
          boxShadow: hovered ? `0 25px 70px ${mission.color}35, inset 0 1px 0 rgba(255,255,255,0.08)` : "0 4px 24px rgba(0,0,0,0.5)",
          ...
          <div style={{
            ...
            borderRadius: "50%",
            background: `radial-gradient(circle, ${mission.color}30 0%, transparent 70%)`,
          ...
          <div style={{
            ...
            background: `${mission.color}18`, border: `1px solid ${mission.color}40`,
            borderRadius: "20px", padding: "3px 10px", width: "fit-content",
            fontSize: "0.65rem", letterSpacing: "2px", color: mission.color,
            fontFamily: "'Orbitron', monospace",
          }}>{mission.tag}</div>
          {/* Year */}
          <div style={{
            fontFamily: "'Orbitron', monospace", fontSize: "2.2rem", fontWeight: "900",
            color: hovered ? mission.color : "rgba(255,255,255,0.92)",
            transition: "color 0.3s", letterSpacing: "1px", lineHeight: 1,
          }}>{mission.year}</div>
          {/* Description */}
          <div style={{ fontSize: "0.82rem", color: "rgba(180,200,255,0.75)", lineHeight: "1.55", fontFamily: "'Inter', sans-serif" }}>
            {mission.title}
          </div>
          {/* Image */}
          <div style={{ flex: 1, borderRadius: "12px", overflow: "hidden", border: "1px solid rgba(255,255,255,0.08)", minHeight: "120px", position: "relative" }}>
            <img src={mission.image} alt={mission.year} style={{
              width: "100%", height: "100%", minHeight: "120px", objectFit: "cover", display: "block",
              filter: hovered ? "brightness(1.15) saturate(1.3)" : "brightness(0.75) saturate(0.8)",
              transform: hovered ? "scale(1.1)" : "scale(1)",
              transition: "all 0.5s ease",
            }} />
            <div style={{
              position: "absolute", inset: 0,
              background: hovered ? `linear-gradient(to top, ${mission.color}30 0%, transparent 60%)` : "linear-gradient(to top, rgba(0,0,0,0.35) 0%, transparent 60%)",
              transition: "all 0.4s ease",
            }} />
            <div style={{ position: "absolute", bottom: "8px", right: "10px", fontSize: "0.65rem", color: "rgba(255,255,255,0.5)", fontFamily: "'Orbitron', monospace" }}>CLICK TO FLIP</div>
          </div>
          {/* Bottom bar */}
          <div style={{
            position: "absolute", bottom: 0, left: 0,
            width: hovered ? "100%" : "35%", height: "2px",
            background: `linear-gradient(90deg, ${mission.color}90, transparent)`,
            transition: "width 0.5s cubic-bezier(0.23,1,0.32,1)", borderRadius: "0 0 0 20px",
          }} />
        </div>
        {/* Back */}
        <div style={{
          ...
          background: `linear-gradient(135deg, ${mission.color}20 0%, rgba(5,10,35,0.97) 100%)`,
          border: `1px solid ${mission.color}50`,
          borderRadius: "20px", padding: "28px",
          display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center",
          gap: "20px", backdropFilter: "blur(14px)",
          boxShadow: `0 20px 60px ${mission.color}25`,
        }}>
          <div style={{ fontSize: "3rem" }}>🌌</div>
          <div style={{ fontFamily: "'Orbitron', monospace", fontSize: "1.6rem", fontWeight: "700", color: mission.color, textAlign: "center" }}>{mission.year}</div>
          <div style={{
            fontSize: "0.9rem", color: "rgba(200,220,255,0.9)", textAlign: "center", lineHeight: "1.65",
            fontFamily: "'Inter', sans-serif", fontStyle: "italic",
          }}>"{mission.fact}"</div>
          <div style={{
            padding: "10px 20px", borderRadius: "30px",
            background: `${mission.color}20`, border: `1px solid ${mission.color}50`,
            fontSize: "0.7rem", letterSpacing: "2px", color: mission.color,
            fontFamily: "'Orbitron', monospace",
          }}>DID YOU KNOW?</div>
          <div style={{ position: "absolute", bottom: "16px", fontSize: "0.65rem", color: "rgba(255,255,255,0.35)", fontFamily: "'Orbitron', monospace" }}>CLICK TO FLIP BACK</div>
        </div>
      </div>
    </div>
  );
...
function StatCard({ stat, isVisible, delay }) {
  ...
  return (
    <div className="stat-card" style={{
      ...
      background: "linear-gradient(135deg, rgba(74,158,255,0.07) 0%, rgba(5,10,35,0.9) 100%)",
      ...
      <div style={{
        ...
        color: "#4a9eff",
        textShadow: counted ? "0 0 30px rgba(74,158,255,0.8)" : "none",
        transition: "text-shadow 0.5s ease",
      }}>{stat.value}</div>
      <div style={{ color: "rgba(150,180,255,0.7)", fontSize: "0.8rem", letterSpacing: "2px", marginTop: "6px", fontFamily: "'Orbitron', monospace" }}>{stat.label}</div>
    </div>
  );
}
...
function FutureCard({ m, delay, isVisible }) {
  ...
  return (
    <div
      ...
      onMouseLeave={() => setHovered(false)}
      style={{
        background: hovered ? `linear-gradient(135deg, ${m.color}15 0%, rgba(5,10,35,0.95) 100%)` : "linear-gradient(135deg, rgba(255,255,255,0.03) 0%, rgba(5,10,35,0.9) 100%)",
        border: `1px solid ${hovered ? m.color + "50" : "rgba(255,255,255,0.07)"}`,
        borderRadius: "16px", padding: "24px",
        transform: isVisible ? (hovered ? "translateY(-8px) scale(1.02)" : "translateY(0) scale(1)") : "translateY(50px) scale(0.9)",
        opacity: isVisible ? 1 : 0,
        transition: `all 0.5s cubic-bezier(0.23,1,0.32,1) ${delay}ms`,
        backdropFilter: "blur(10px)",
        boxShadow: hovered ? `0 20px 50px ${m.color}25` : "none",
        cursor: "default",
      }}
    >
      <div style={{ fontSize: "2rem", marginBottom: "10px" }}>{m.icon}</div>
      <div style={{ fontFamily: "'Orbitron', monospace", fontSize: "1.1rem", fontWeight: "700", color: m.color, marginBottom: "6px" }}>{m.year}</div>
      <div style={{ fontFamily: "'Orbitron', monospace", fontSize: "0.85rem", fontWeight: "600", color: "rgba(255,255,255,0.9)", marginBottom: "10px" }}>{m.title}</div>
      <div style={{ fontSize: "0.82rem", color: "rgba(160,185,255,0.7)", lineHeight: "1.6", fontFamily: "'Inter', sans-serif" }}>{m.desc}</div>
    </div>
  );
}
...
function SolarSystem() {
  const planets = [
    { name: "Mercury", color: "#b5b5b5", size: 8, orbit: 60, speed: 4.7, emoji: "⚫" },
    { name: "Venus", color: "#e8cda0", size: 14, orbit: 90, speed: 3.5, emoji: "🟡" },
    { name: "Earth", color: "#4a9eff", size: 15, orbit: 125, speed: 2.9, emoji: "🌍" },
    { name: "Mars", color: "#c1440e", size: 10, orbit: 165, speed: 2.4, emoji: "🔴" },
  ];
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: "radial-gradient(circle, #fff 0%, #ffdd44 40%, #ff8800 100%)",
        boxShadow: "0 0 40px #ffdd44, 0 0 80px rgba(255,200,0,0.3)",
        zIndex: 5,
      }} />
      {planets.map((p, i) => (
        <div key={p.name} style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <div style={{
            ...
            width: p.orbit * 2, height: p.orbit * 2,
            borderRadius: "50%",
            animation: `orbit${i} ${10 - p.speed}s linear infinite`,
          }}>
            <div style={{
              position: "absolute", top: "0", left: "50%",
              transform: "translateX(-50%)",
              width: p.size, height: p.size, borderRadius: "50%",
              background: `radial-gradient(circle, ${p.color} 0%, ${p.color}88 100%)`,
    ...
    <div style={{ display: "flex", gap: "16px", justifyContent: "center", flexWrap: "wrap", marginTop: "32px" }}>
      {units.map(([label, val]) => (
        <div key={label} style={{
          ...
          borderRadius: "12px", padding: "16px 12px", backdropFilter: "blur(8px)",
        }}>
          <div style={{ fontFamily: "'Orbitron', monospace", fontSize: "2rem", fontWeight: "900", color: "#4a9eff", lineHeight: 1 }}>
            {String(val).padStart(2, "0")}
          </div>
          <div style={{ fontSize: "0.6rem", letterSpacing: "2px", color: "rgba(150,180,255,0.6)", marginTop: "4px", fontFamily: "'Orbitron', monospace" }}>{label}</div>
        </div>
      ))}
    </div>
  );
...
function QuoteRotator() {
  ...
  return (
    <div style={{
      ...
      background: "linear-gradient(135deg, rgba(74,158,255,0.05) 0%, rgba(5,10,40,0.85) 100%)",
      border: "1px solid rgba(74,158,255,0.12)", borderRadius: "24px", backdropFilter: "blur(10px)",
      opacity: fade ? 1 : 0, transition: "opacity 0.4s ease",
    }}>
      <div style={{ fontSize: "3rem", marginBottom: "16px", opacity: 0.4 }}>"</div>
      <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "clamp(1rem, 2.5vw, 1.4rem)", fontStyle: "italic", color: "rgba(200,220,255,0.9)", lineHeight: "1.7", maxWidth: "700px", margin: "0 auto 20px" }}>{q.text}</p>
      <div style={{ fontFamily: "'Orbitron', monospace", fontSize: "0.75rem", letterSpacing: "3px", color: "#4a9eff" }}>— {q.author}</div>
    </div>
  );
}
...
export default function MissionSection() {
  ...
  useEffect(() => {
    const observers = cardRefs.current.map((ref, i) => {
      if (!ref) return null;
      const obs = new IntersectionObserver(([e]) => { if (e.isIntersecting) setVisibleCards(p => new Set([...p, i])); }, { threshold: 0.1, rootMargin: "0px 0px -40px 0px" });
      obs.observe(ref); return obs;
    });
    const sObs = new IntersectionObserver(([e]) => { if (e.isIntersecting) setStatsVisible(true); }, { threshold: 0.2 });
    if (statsRef.current) sObs.observe(statsRef.current);
    const fObs = new IntersectionObserver(([e]) => { if (e.isIntersecting) setFutureVisible(true); }, { threshold: 0.1 });
    if (futureRef.current) fObs.observe(futureRef.current);
    const qObs = new IntersectionObserver(([e]) => { if (e.isIntersecting) setQuoteVisible(true); }, { threshold: 0.2 });
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .shimmer-text { background: linear-gradient(90deg, #4a9eff, #fff, #4a9eff, #7bc8ff); background-size:300% auto; -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; animation:shimmer 4s linear infinite; }
        .float-el { animation: float 7s ease-in-out infinite; }
        .row-divider { position:relative; display:flex; align-items:center; margin:20px 0; }
        ...
        .dot-pulse { width:8px; height:8px; border-radius:50%; background:#4a9eff; margin:0 12px; animation:pulseGlow 2s ease-in-out infinite; box-shadow:0 0 10px #4a9eff; }
        .stat-card:hover { transform: translateY(-6px) scale(1.03) !important; box-shadow: 0 20px 50px rgba(74,158,255,0.2) !important; border-color: rgba(74,158,255,0.35) !important; }
        .stat-card { transition: all 0.3s ease !important; }
        .scan-line { position:fixed; top:0; left:0; width:100%; height:2px; background:linear-gradient(90deg,transparent,rgba(74,158,255,0.3),transparent); animation:scanLine 8s linear infinite; pointer-events:none; z-index:100; }
      `}</style>
        ...
        <div style={{ position: "relative", zIndex: 2, maxWidth: "1200px", margin: "0 auto", padding: "0 32px" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: "80px" }}>
            <div className="title-a1" style={{
              ...
              padding: "8px 24px", fontSize: "0.75rem", letterSpacing: "5px",
            ...
            <h2 className="title-a3" style={{
              ...
              color: "rgba(150,180,255,0.65)", letterSpacing: "8px", marginBottom: "24px",
            ...
            <div className="title-a4" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "14px", marginTop: "40px" }}>
              {[80, 30].map((w, i) => <div key={i} style={{ width: w, height: "1px", background: `linear-gradient(${i ? "270deg" : "90deg"}, transparent, #4a9eff)` }} />)}
              {[6, 10, 6].map((s, i) => <div key={i} style={{ width: s, height: s, borderRadius: "50%", background: s === 10 ? "#fff" : "#4a9eff", boxShadow: `0 0 ${s === 10 ? 20 : 12}px #4a9eff` }} />)}
          ...
          {/* ── SOLAR SYSTEM + INTRO ── */}
          <div style={{
            marginBottom: "80px", padding: "50px 40px",
            background: "linear-gradient(135deg, rgba(74,158,255,0.05) 0%, rgba(5,10,40,0.85) 100%)",
            border: "1px solid rgba(74,158,255,0.12)", borderRadius: "24px", backdropFilter: "blur(12px)",
          }}>
            <div className="solar-hero">
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: "'Orbitron', monospace", fontSize: "0.7rem", letterSpacing: "4px", color: "#4a9eff", marginBottom: "16px" }}>🌌 OUR SOLAR SYSTEM</div>
                <h3 style={{ fontFamily: "'Orbitron', monospace", fontSize: "clamp(1.5rem, 3vw, 2.5rem)", fontWeight: "700", color: "white", lineHeight: 1.2, marginBottom: "20px" }}>
                  Exploring<br /><span style={{ color: "#4a9eff" }}>Every Corner</span>
                </h3>
                <p style={{ color: "rgba(160,185,255,0.7)", fontSize: "0.95rem", lineHeight: "1.8", maxWidth: "440px" }}>
                ...
                <div style={{ display: "flex", gap: "24px", marginTop: "28px", flexWrap: "wrap" }}>
                  {[["8", "Planets Explored"], ["500+", "Missions Flown"], ["23B km", "Farthest Object"]].map(([v, l]) => (
                    <div key={l} style={{ textAlign: "center" }}>
                      <div style={{ fontFamily: "'Orbitron', monospace", fontSize: "1.4rem", fontWeight: "700", color: "#4a9eff" }}>{v}</div>
                      <div style={{ fontSize: "0.7rem", color: "rgba(150,180,255,0.6)", letterSpacing: "1px" }}>{l}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div style={{ marginBottom: "56px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px", color: "rgba(150,180,255,0.45)", fontSize: "0.65rem", letterSpacing: "2px", fontFamily: "'Orbitron', monospace" }}>
              <span>1957</span><span style={{ color: "#4a9eff" }}>TIMELINE PROGRESS — {Math.round(progress * 100)}%</span><span>2020s</span>
            </div>
            <div style={{ height: "3px", background: "rgba(74,158,255,0.1)", borderRadius: "2px", overflow: "hidden", position: "relative" }}>
              <div style={{ height: "100%", width: `${progress * 100}%`, background: "linear-gradient(90deg, #1a3a8a, #4a9eff, #7bc8ff, #fff)", borderRadius: "2px", transition: "width 0.12s ease", boxShadow: "0 0 12px #4a9eff, 0 0 24px rgba(74,158,255,0.4)" }} />
              <div style={{ position: "absolute", right: `${100 - progress * 100}%`, top: "50%", transform: "translate(50%, -50%)", width: "8px", height: "8px", borderRadius: "50%", background: "#fff", boxShadow: "0 0 12px #4a9eff", transition: "right 0.12s ease" }} />
            </div>
          </div>
          ...
          <div ref={quoteRef} style={{
            marginTop: "80px",
            opacity: quoteVisible ? 1 : 0, transform: quoteVisible ? "translateY(0)" : "translateY(40px)",
          ...
          <div ref={statsRef} style={{ marginTop: "80px" }}>
            <div style={{ textAlign: "center", marginBottom: "40px" }}>
              <div style={{ fontFamily: "'Orbitron', monospace", fontSize: "0.7rem", letterSpacing: "4px", color: "#4a9eff", marginBottom: "12px" }}>📊 BY THE NUMBERS</div>
              <h3 style={{ fontFamily: "'Orbitron', monospace", fontSize: "clamp(1.5rem, 3vw, 2.2rem)", fontWeight: "700", color: "white" }}>Space <span style={{ color: "#4a9eff" }}>Milestones</span></h3>
            </div>
            <div className="stats-grid">
              {stats.map((s, i) => <StatCard key={s.label} stat={s} isVisible={statsVisible} delay={i * 120} />)}
            </div>
          </div>

          {/* ── FUTURE MISSIONS ── */}
          <div ref={futureRef} style={{ marginTop: "80px" }}>
            <div style={{ textAlign: "center", marginBottom: "40px" }}>
              <div style={{ fontFamily: "'Orbitron', monospace", fontSize: "0.7rem", letterSpacing: "4px", color: "#ff6b6b", marginBottom: "12px" }}>🚀 WHAT LIES AHEAD</div>
              <h3 style={{ fontFamily: "'Orbitron', monospace", fontSize: "clamp(1.5rem, 3vw, 2.2rem)", fontWeight: "700", color: "white" }}>Future <span style={{ color: "#ff6b6b" }}>Missions</span></h3>
              <p style={{ color: "rgba(160,185,255,0.6)", fontSize: "0.9rem", marginTop: "12px", maxWidth: "500px", margin: "12px auto 0" }}>
                The next chapters of space exploration are already being written.
              </p>
            </div>
            <div className="future-grid">
              {futureMs.map((m, i) => <FutureCard key={m.year} m={m} delay={i * 100} isVisible={futureVisible} />)}
            </div>
          </div>

          {/* ── ARTEMIS COUNTDOWN ── */}
          <div style={{
            marginTop: "80px", padding: "50px 40px", textAlign: "center",
            background: "linear-gradient(135deg, rgba(74,158,255,0.06) 0%, rgba(5,10,40,0.88) 100%)",
            border: "1px solid rgba(74,158,255,0.15)", borderRadius: "24px", backdropFilter: "blur(12px)",
          }}>
            <div style={{ fontFamily: "'Orbitron', monospace", fontSize: "0.7rem", letterSpacing: "4px", color: "#4a9eff", marginBottom: "16px" }}>⏱ NEXT LAUNCH WINDOW</div>
            <h3 style={{ fontFamily: "'Orbitron', monospace", fontSize: "clamp(1.3rem, 3vw, 2rem)", fontWeight: "700", color: "white", marginBottom: "8px" }}>
              Artemis III — <span style={{ color: "#4a9eff" }}>Moon Landing</span>
            </h3>
            <p style={{ color: "rgba(160,185,255,0.6)", fontSize: "0.85rem", maxWidth: "400px", margin: "0 auto" }}>Target: September 2026 — First crewed lunar landing since 1972</p>
            <Countdown />
          </div>

          {/* ── BOTTOM CTA ── */}
          <div style={{
            textAlign: "center", marginTop: "80px", padding: "70px 40px",
            background: "linear-gradient(135deg, rgba(74,158,255,0.06) 0%, rgba(10,20,60,0.7) 100%)",
            border: "1px solid rgba(74,158,255,0.12)", borderRadius: "24px", backdropFilter: "blur(12px)",
            ...
            <div style={{ position: "relative", zIndex: 1 }}>
              ...
              <h3 style={{ fontFamily: "'Orbitron', monospace", fontSize: "clamp(1.8rem, 4vw, 3rem)", fontWeight: "700", color: "white", marginBottom: "16px" }}>
                What's <span style={{ color: "#4a9eff" }}>Next?</span>
              </h3>
              <p style={{ color: "rgba(150,180,220,0.65)", fontSize: "1rem", maxWidth: "480px", margin: "0 auto 36px", lineHeight: "1.75" }}>
              ...
              <div style={{ display: "flex", gap: "16px", justifyContent: "center", flexWrap: "wrap" }}>
                {[["EXPLORE MISSIONS →", "transparent", "#4a9eff"], ["FUTURE LAUNCHES", "rgba(74,158,255,0.15)", "#7bc8ff"]].map(([label, bg, col]) => (
                  <button key={label}
                    onMouseEnter={e => { e.target.style.background = col; e.target.style.color = "#000"; e.target.style.transform = "translateY(-4px)"; e.target.style.boxShadow = `0 16px 40px ${col}50`; }}
                    onMouseLeave={e => { e.target.style.background = bg; e.target.style.color = col; e.target.style.transform = "translateY(0)"; e.target.style.boxShadow = "none"; }}
                    style={{
                      background: bg, border: `2px solid ${col}`, borderRadius: "50px",
                      padding: "14px 36px", color: col,
                      fontFamily: "'Orbitron', monospace", fontSize: "0.8rem", letterSpacing: "3px",
          ...
          {/* ── FOOTER ── */}
          <div style={{ textAlign: "center", marginTop: "60px", paddingTop: "30px", borderTop: "1px solid rgba(74,158,255,0.1)" }}>
